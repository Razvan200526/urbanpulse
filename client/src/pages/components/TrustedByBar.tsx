...
const ids = Array.from({ length: allLogos.length }, () => crypto.randomUUID());
...
export const TrustedByBar = () => {
	return (
		<motion.div
			...
			className="py-10"
		>
			<p className="text-center text-xs font-semibold uppercase tracking-widest text-accent mb-6">
			...
			<div className="relative overflow-hidden">
				...
				<div className="flex items-center gap-12 animate-marquee">
					{allLogos.map((logo, idx) => (
						<div
							key={`${ids[idx]}`}
							className="shrink-0 text-muted/30 text-sm font-semibold tracking-wide hover:text-secondary-text transition-colors duration-300 select-none whitespace-nowrap"
						>
							{logo.text}
						</div>
					))}
