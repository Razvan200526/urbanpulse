...
class User(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "user_home_location_spatial_index", "homeLocation", postgresql_using="gist"
        ),
        Index(
            "user_last_known_location_spatial_index",
            "lastKnownLocation",
    ...
    heroAlertRadiusMeters: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default=text("500")
    )
    image: Mapped[Optional[str]] = mapped_column(Text)
    role: Mapped[Optional[str]] = mapped_column(
        Text, server_default=text("'user'::text")
    )
    bio: Mapped[Optional[str]] = mapped_column(Text)
    trustScore: Mapped[Optional[float]] = mapped_column(
        Double(53), server_default=text("0")
    )
    successfulInteractions: Mapped[Optional[int]] = mapped_column(
        Integer, server_default=text("0")
    )
    isVerified: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("false")
    )
    rememberMe: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("false")
    )
    banned: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("false")
    )
    banReason: Mapped[Optional[str]] = mapped_column(Text)
    banExpires: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    homeLocation: Mapped[Optional[Any]] = mapped_column(
        Geometry("POINT", dimension=2, from_text="ST_GeomFromEWKT", name="geometry")
    )
    lastKnownLocation: Mapped[Optional[Any]] = mapped_column(
        Geometry("POINT", dimension=2, from_text="ST_GeomFromEWKT", name="geometry")
    )
    lastKnownLocationUpdatedAt: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime
    )
    ...
    quiet_hours: Mapped[list["QuietHours"]] = relationship(
        "QuietHours", back_populates="user"
    )
    resources: Mapped[list["Resources"]] = relationship(
        "Resources", back_populates="user"
    )
    ...
    pulse_confirmation: Mapped[list["PulseConfirmation"]] = relationship(
        "PulseConfirmation", back_populates="user"
    )
    report_reporterId: Mapped[list["Report"]] = relationship(
        "Report", foreign_keys="[Report.reporterId]", back_populates="user"
    )
    report_targetUserId: Mapped[list["Report"]] = relationship(
        "Report", foreign_keys="[Report.targetUserId]", back_populates="user_"
    )
    response: Mapped[list["Response"]] = relationship("Response", back_populates="user")
    transaction_borrowerId: Mapped[list["Transaction"]] = relationship(
        "Transaction", foreign_keys="[Transaction.borrowerId]", back_populates="user"
    )
    transaction_lenderId: Mapped[list["Transaction"]] = relationship(
        "Transaction", foreign_keys="[Transaction.lenderId]", back_populates="user_"
    )
    conversation_member: Mapped[list["ConversationMember"]] = relationship(
        "ConversationMember", back_populates="user"
    )
    message: Mapped[list["Message"]] = relationship("Message", back_populates="user")
    resource_review_revieweeId: Mapped[list["ResourceReview"]] = relationship(
        "ResourceReview",
        foreign_keys="[ResourceReview.revieweeId]",
        back_populates="user",
    )
    resource_review_reviewerId: Mapped[list["ResourceReview"]] = relationship(
        "ResourceReview",
        foreign_keys="[ResourceReview.reviewerId]",
        back_populates="user_",
    )
    message_receipt: Mapped[list["MessageReceipt"]] = relationship(
        "MessageReceipt", back_populates="user"
    )
...
class Pulse(Base):
    ...
    type: Mapped[str] = mapped_column(
        Text, nullable=False, server_default=text("'Emergency'::text")
    )
    userId: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    pulseUploadState: Mapped[str] = mapped_column(
        Text, nullable=False, server_default=text("'pending'::text")
    )
    imageUrls: Mapped[list[str]] = mapped_column(
        ARRAY(Text()), nullable=False, server_default=text("'{}'::text[]")
    )
    isResolved: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("false")
    )
    createdAt: Mapped[datetime.datetime] = mapped_column(
        DateTime, nullable=False, server_default=text("now()")
    )
    requestedSkillTags: Mapped[list[str]] = mapped_column(
        ARRAY(Text()), nullable=False, server_default=text("'{}'::text[]")
    )
    matchMetadata: Mapped[dict] = mapped_column(
    ...
    conversation: Mapped[list["Conversation"]] = relationship(
        "Conversation", back_populates="pulse"
    )
    pet_alert: Mapped[list["PetAlert"]] = relationship(
        "PetAlert", back_populates="pulse"
    )
    pulse_confirmation: Mapped[list["PulseConfirmation"]] = relationship(
        "PulseConfirmation", back_populates="pulse"
    )
    report: Mapped[list["Report"]] = relationship("Report", back_populates="pulse")
    response: Mapped[list["Response"]] = relationship(
        "Response", back_populates="pulse"
    )
...
class QuietHours(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.user.id"],
            ondelete="CASCADE",
            name="quiet_hours_userId_user_id_fk",
        ),
...
class Resources(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.user.id"],
            ondelete="CASCADE",
            name="resources_userId_user_id_fk",
        ),
    ...
    resource_review: Mapped[list["ResourceReview"]] = relationship(
        "ResourceReview", back_populates="resources"
    )
...
class Conversation(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.pulse.id"],
            ondelete="SET NULL",
            name="conversation_pulseId_pulse_id_fk",
        ),
    ...
    pulse: Mapped[Optional["Pulse"]] = relationship(
        "Pulse", back_populates="conversation"
    )
    conversation_member: Mapped[list["ConversationMember"]] = relationship(
...
class PetAlert(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.pulse.id"],
            ondelete="CASCADE",
            name="pet_alert_pulseId_pulse_id_fk",
        ),
        PrimaryKeyConstraint("id", name="pet_alert_pkey"),
        Index(
            "pet_alert_image_embedding_cosine_idx",
            "imageEmbedding",
            postgresql_ops={"imageEmbedding": "vector_cosine_ops"},
    ...
    embeddingStatus: Mapped[str] = mapped_column(
        Text, nullable=False, server_default=text("'pending'::text")
    )
    ...
    pet_match_foundAlertId: Mapped[list["PetMatch"]] = relationship(
        "PetMatch", foreign_keys="[PetMatch.foundAlertId]", back_populates="pet_alert"
    )
    pet_match_lostAlertId: Mapped[list["PetMatch"]] = relationship(
        "PetMatch", foreign_keys="[PetMatch.lostAlertId]", back_populates="pet_alert_"
    )
...
class PulseConfirmation(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.pulse.id"],
            ondelete="CASCADE",
            name="pulse_confirmation_pulseId_pulse_id_fk",
        ),
        ForeignKeyConstraint(
            ["userId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="pulse_confirmation_userId_user_id_fk",
        ),
    ...
    pulseId: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    userId: Mapped[str] = mapped_column(Text, nullable=False)
    confirmedAt: Mapped[datetime.datetime] = mapped_column(
        DateTime, nullable=False, server_default=text("now()")
    )
...
class Report(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["public.pulse.id"],
            ondelete="CASCADE",
            name="report_targetPulseId_pulse_id_fk",
        ),
        ForeignKeyConstraint(
            ["targetUserId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="report_targetUserId_user_id_fk",
        ),
    ...
    user: Mapped["User"] = relationship(
        "User", foreign_keys=[reporterId], back_populates="report_reporterId"
    )
    pulse: Mapped[Optional["Pulse"]] = relationship("Pulse", back_populates="report")
    user_: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[targetUserId], back_populates="report_targetUserId"
    )
...
class Response(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.pulse.id"],
            ondelete="CASCADE",
            name="response_pulseId_pulse_id_fk",
        ),
        ForeignKeyConstraint(
            ["responderId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="response_responderId_user_id_fk",
        ),
...
class Transaction(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.user.id"],
            ondelete="SET NULL",
            name="transaction_borrowerId_user_id_fk",
        ),
        ForeignKeyConstraint(
            ["lenderId"],
            ["public.user.id"],
            ondelete="SET NULL",
            name="transaction_lenderId_user_id_fk",
        ),
        ForeignKeyConstraint(
            ["resourceId"],
            ["public.resources.id"],
            ondelete="CASCADE",
            name="transaction_resourceId_resources_id_fk",
        ),
    ...
    user: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[borrowerId], back_populates="transaction_borrowerId"
    )
    user_: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[lenderId], back_populates="transaction_lenderId"
    )
    resources: Mapped["Resources"] = relationship(
        "Resources", back_populates="transaction"
    )
    resource_review: Mapped["ResourceReview"] = relationship(
        "ResourceReview", uselist=False, back_populates="transaction"
    )
...
class ConversationMember(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            name="conversation_member_conversationId_conversation_id_fk",
        ),
        ForeignKeyConstraint(
            ["userId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="conversation_member_userId_user_id_fk",
        ),
    ...
    conversation: Mapped["Conversation"] = relationship(
        "Conversation", back_populates="conversation_member"
    )
...
class Message(Base):
    ...
    sentAt: Mapped[datetime.datetime] = mapped_column(
        DateTime, nullable=False, server_default=text("now()")
    )
...
class PetMatch(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.pet_alert.id"],
            ondelete="CASCADE",
            name="pet_match_foundAlertId_pet_alert_id_fk",
        ),
        ForeignKeyConstraint(
            ["lostAlertId"],
            ["public.pet_alert.id"],
            ondelete="CASCADE",
            name="pet_match_lostAlertId_pet_alert_id_fk",
        ),
        PrimaryKeyConstraint("id", name="pet_match_pkey"),
        Index(
            "pet_match_lost_found_unique", "lostAlertId", "foundAlertId", unique=True
        ),
        {"schema": "public"},
    )
    ...
    matchedAttributes: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, server_default=text("'[]'::jsonb")
    )
    status: Mapped[str] = mapped_column(
    ...
    pet_alert: Mapped["PetAlert"] = relationship(
        "PetAlert", foreign_keys=[foundAlertId], back_populates="pet_match_foundAlertId"
    )
    pet_alert_: Mapped["PetAlert"] = relationship(
        "PetAlert", foreign_keys=[lostAlertId], back_populates="pet_match_lostAlertId"
    )
...
class ResourceReview(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.resources.id"],
            ondelete="CASCADE",
            name="resource_review_resourceId_resources_id_fk",
        ),
        ForeignKeyConstraint(
            ["revieweeId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="resource_review_revieweeId_user_id_fk",
        ),
        ForeignKeyConstraint(
            ["reviewerId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="resource_review_reviewerId_user_id_fk",
        ),
        ForeignKeyConstraint(
            ["transactionId"],
            ["public.transaction.id"],
            ondelete="CASCADE",
            name="resource_review_transactionId_transaction_id_fk",
        ),
    ...
    resources: Mapped["Resources"] = relationship(
        "Resources", back_populates="resource_review"
    )
    user: Mapped["User"] = relationship(
        "User", foreign_keys=[revieweeId], back_populates="resource_review_revieweeId"
    )
    user_: Mapped["User"] = relationship(
        "User", foreign_keys=[reviewerId], back_populates="resource_review_reviewerId"
    )
    transaction: Mapped["Transaction"] = relationship(
        "Transaction", back_populates="resource_review"
    )
...
class MessageReceipt(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["public.message.id"],
            ondelete="CASCADE",
            name="message_receipt_messageId_message_id_fk",
        ),
        ForeignKeyConstraint(
            ["userId"],
            ["public.user.id"],
            ondelete="CASCADE",
            name="message_receipt_userId_user_id_fk",
        ),
        PrimaryKeyConstraint("id", name="message_receipt_pkey"),
        Index(
            "message_receipt_message_user_unique", "messageId", "userId", unique=True
        ),
    ...
    message: Mapped["Message"] = relationship(
        "Message", back_populates="message_receipt"
    )
